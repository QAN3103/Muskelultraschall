...
def calculate_glcm(image, distances, angles, levels):
    ...
    entropy_values = []
    for angle in angles:
        entropy = -np.sum(glcm[:, :, 0, angles.index(angle)] * np.log2(glcm[:, :, 0, angles.index(angle)] + (glcm[:, :, 0, angles.index(angle)] == 0)))
...
def process_images(image_ids, root_folder, output_csv, original_df):
    ...
    for image_id in image_ids:
        ...
        if bone_image is not None:
            ...
            angles = [0, np.pi/4, np.pi/2, 3*np.pi/4]  # four angles of glcm, 0, 45, 90, 135.
            glcm_features = calculate_glcm(bone_image, distances=[1], angles=angles, levels=256)
        ...
        if muscle_image is not None:
            ...
            glcm_features = calculate_glcm(muscle_image, distances=[1], angles=angles, levels=256)
...
def main():
    ...
    output_csv = '/work/Train/Features_train_mean.csv'
